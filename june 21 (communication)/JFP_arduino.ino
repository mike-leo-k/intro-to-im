...
int poten = A0;    // analog sensor
int blue = 2;   // first digital sensor
int red = 4;    // second digital sensor
...
void setup() {
  ...
  pinMode(blue, INPUT);   // digital sensor is on digital pin 2
  pinMode(red, INPUT);   // digital sensor is on digital pin 4
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    turning = analogRead(poten);
    turning = map(turning, 0, 1023, 0, 255);
    // delay 10ms to let the ADC recover:
    delay(100);
 
    portal = digitalRead(blue);
    delay(50);
    
    bullet = digitalRead(red);
    delay(50);
    // send sensor values:
